...
export const Product = ({ addToCart, productID ,display_cart }) => {
...
useEffect(()=>{
  const fetchProduct = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://fashorabe26.onrender.com/getbycategory/${category}`)
      if (response.data) {
        setFetured(response.data)
        } else {
          console.log("No product found.");
          }
          } catch (e) {
...
<div className="p-4 md:mt-40 flex flex-col w-screen mt-30 max-w-[80%] md:mx-auto">
  ...
  <div className="md:max-w-[100vh]   md:p-0 ">
    <ImageGallery
      ...
      autoPlay={true}
      slideInterval={3000}
      renderItem={({ original, thumbnail }) => (
        <div className=" "> {/* Fixed height for images */}
          <img
            src={original}
            alt="Gallery Image"
  ...
  <p>No images available</p>
          ...
          <div className="flex flex-col md:w-[50%] items-start md:p-6 p-3">
            ...
            <div className="md:flex md:flex-row flex-row md:gap-10 md:my-10 my-4">
              <div className="text-2xl items-center flex md:flex-row title_paragraph">
                Size
              </div>
              {/* Size Selection */}
              {/* You can uncomment this when size options are available */}
            </div>
            ...
            <div className="flex md:flex-row md:w-[100%] justify-between gap-4 flex-col my-9">
              <div className="flex md:flex-row">
                <button
                  ...
                  className={`md:py-3 py-4 px-4 text-xl md:px-12 md:text-2xl flex border-4 border-black text-center ${added==="ITEM ADDED"? "bg-orange-600 hover:brightness-160 text-white rounded-xl border-white":""} `}
                >
      ...
      <div className="flex flex-col  w-screen mt-30 max-w-[80%] md:mx-auto border-t-2 border-gray-500 pt-10 mb-20 p-4 ml-9">
        ...
        <div>
      <Swiper  spaceBetween={20} grabCursor={true} grid={{
